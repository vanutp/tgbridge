...
async function telegramToMinecraft(server: Server) {
  ...
  await delay(300)
  assert(
    server.client.findMessage((msg) => JSON.stringify(msg.json).includes(text)),
  )
}

async function advancement(server: Server) {
  const resp = await server.rcon.send(
...
async function reloadCommandFromOp(server: Server) {
  ...
  await delay(300)
  assert(
    server.client.findMessage((msg) =>
      JSON.stringify(msg.json).includes('Config reloaded.')
    ),
  )
}
...
async function muteUnmuteCommand(server: Server) {
  ...
  assert(
    !server.client.findMessage((msg) =>
      JSON.stringify(msg.json).includes(textMuted)
    ),
  )
  ...
  assert(
    server.client.findMessage((msg) =>
      JSON.stringify(msg.json).includes(textUnmuted)
    ),
  )
}
...
async function joinAndLeavePlayer(server: Server) {
  ...
  assert(
    server.tg.findMessage((msg) =>
      (msg.text ?? '').includes(`${secondClient.username} joined the game`)
    ),
  )
